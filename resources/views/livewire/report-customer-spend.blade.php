<div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm flex flex-col space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <h4 class="text-sm font-bold text-slate-900 uppercase tracking-wider font-sans">Customer Spend Analysis</h4>
            <p class="text-xs text-slate-400 mt-0.5 font-sans">Analyze the highest and lowest spending customers.</p>
        </div>
    ...
    <div class="flex border-b border-slate-100 p-0.5 bg-slate-50/80 rounded-2xl">
        <button type="button" 
                ...
                class="flex-1 py-2 text-xs font-bold rounded-xl transition duration-150 flex items-center justify-center gap-1.5 {{ $viewMode === 'highest' ? 'bg-white text-indigo-600 shadow-sm border border-slate-200/40' : 'text-slate-500 hover:text-slate-700' }}">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>
            Highest Spenders
        </button>
        <button type="button" 
                wire:click="setViewMode('lowest')"
                class="flex-1 py-2 text-xs font-bold rounded-xl transition duration-150 flex items-center justify-center gap-1.5 {{ $viewMode === 'lowest' ? 'bg-white text-rose-600 shadow-sm border border-slate-200/40' : 'text-slate-500 hover:text-slate-700' }}">
                ...
                @foreach($customers as $customer)
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center justify-between text-xs">
                            <div>
                                <a href="{{ route('admin.users.show', $customer->id) }}" wire:navigate class="font-extrabold text-slate-800 hover:underline">{{ $customer->name }}</a>
                                <span class="text-[10px] text-slate-400 block">{{ $customer->email }}</span>
                            </div>
                            <div class="text-right">
                                <span class="font-bold text-slate-900">${{ number_format($customer->total_spend, 2) }}</span>
                                <span class="text-[10px] text-slate-400 block">{{ $customer->orders_count }} orders</span>
                            </div>
                        </div>
                        <div class="h-2 w-full bg-slate-50 rounded-full overflow-hidden border border-slate-100 flex">
