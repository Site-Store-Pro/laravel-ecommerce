                ...
                <div id="inline-tinymce-editor-section" class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border-2 border-indigo-500 p-6 space-y-6 transition-all">
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    <h4 class="text-xs font-extrabold uppercase tracking-wider text-slate-800 dark:text-white flex items-center gap-2">
                                        ...
                                        <span>Block Language Translations</span>
                                    </h4>
                                    <p class="text-2xs text-slate-500 dark:text-slate-400 mt-0.5">View, edit, and translate this block's title and content for each active non-default language.</p>
                                </div>
                                ...
                                <div class="flex items-center gap-1.5 flex-wrap">
                                    @foreach($activeLangs as $lang)
                                        <button type="button"
                                            ...
                                            @php
                                                $tRec = \App\Models\CmsBuilderBlockTranslation::where('cms_builder_block_id', $editingBlockId)->where('language_id', $lang->id)->first();
                                            @endphp
                                            @if($tRec)
                            ...
                            @if($activeLangCode)
                                <div class="p-4 bg-slate-50 dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-4">
                                    ...
                                    <div class="flex items-center justify-between flex-wrap gap-2">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <button type="button"
                                                    wire:click="aiTranslateBlockInline"
                                                ...
                                                <span wire:loading wire:target="aiTranslateBlockInline" class="animate-spin inline-block w-3 h-3 border-2 border-violet-500 border-t-transparent rounded-full"></span>
                                                <svg wire:loading.remove wire:target="aiTranslateBlockInline" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                                                ✦ Generate AI Translation
                                            </button>
                                            <button type="button"
                                                    wire:click="autoTranslateBlock"
                                                    wire:loading.attr="disabled"
                                                ...
                                                <span wire:loading wire:target="autoTranslateBlock" class="animate-spin inline-block w-3 h-3 border-2 border-amber-500 border-t-transparent rounded-full"></span>
                                                <svg wire:loading.remove wire:target="autoTranslateBlock" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129"/></svg>
                                                Queue Bulk Job
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            <label class="block text-2xs font-bold text-slate-500 uppercase tracking-wider mb-1">Translated Block Title</label>
                                            <input type="text" wire:model="trans_title" placeholder="Translated block title..." class="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-800 dark:text-slate-100 focus:outline-none focus:border-indigo-500">
                                        </div>

                                        <div>
                                            <label class="block text-2xs font-bold text-slate-500 uppercase tracking-wider mb-1">Desktop Content (HTML)</label>
                                            <textarea wire:model="trans_content_desktop" rows="4" placeholder="Translated HTML content for desktop view..." class="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-800 dark:text-slate-100 font-mono focus:outline-none focus:border-indigo-500"></textarea>
                                        </div>

                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div>
                                                <label class="block text-2xs font-bold text-slate-500 uppercase tracking-wider mb-1">Tablet Content (Optional Override)</label>
                                                <textarea wire:model="trans_content_tablet" rows="3" placeholder="Leave empty to use desktop translation..." class="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-800 dark:text-slate-100 font-mono focus:outline-none focus:border-indigo-500"></textarea>
                                            </div>

                                            <div>
                                                <label class="block text-2xs font-bold text-slate-500 uppercase tracking-wider mb-1">Mobile Content (Optional Override)</label>
                                                <textarea wire:model="trans_content_mobile" rows="3" placeholder="Leave empty to use desktop translation..." class="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-800 dark:text-slate-100 font-mono focus:outline-none focus:border-indigo-500"></textarea>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="flex items-center justify-between gap-3 pt-4 border-t border-slate-100 dark:border-slate-700">
                        ...
                        <button wire:click="translateBlock({{ $editingBlockId }})"
                            ...
                            <span wire:loading.remove wire:target="translateBlock">Queue All Languages Bulk Job</span>
