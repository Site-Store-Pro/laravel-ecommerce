        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-8">
            <div>
                ...
                <p class="mt-2 text-sm text-slate-500">
                    Manage tags to tag your pages and posts.
                </p>
            </div>
            <div>
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-slate-700 text-sm">
                                    @forelse($tags as $tag)
                                        <tr class="hover:bg-slate-50/20 dark:hover:bg-slate-700/10 transition-colors">
                                            <td class="py-4 font-bold text-slate-800 dark:text-slate-200">{{ $tag->name }}</td>
                                            <td class="py-4 text-slate-500 dark:text-slate-400">
                                                <div class="flex items-center gap-1.5">
                                                    <span>{{ $tag->slug }}</span>
                                                    <a href="{{ route('cms.tag', $tag->slug) }}" target="_blank" title="View Public Landing Page" class="text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors inline-flex">
                                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                                                    </a>
                                                </div>
                                            </td>
                                            <td class="py-4 text-right space-x-2">
                                                <button wire:click="openForm({{ $tag->id }})" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 font-semibold">Edit</button>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="3" class="py-8 text-center text-slate-400 dark:text-slate-500 font-medium">No tags created yet.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                    ...
                    @if($isFormOpen)
                        <div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-3xl p-6 shadow-sm h-fit">
                            ...
                            <form wire:submit.prevent="save" class="space-y-6">
                                <div>
                                    <label class="text-xs font-bold text-slate-400 dark:text-slate-500 block mb-2 uppercase tracking-wider">Tag Name</label>
                                    <input type="text" wire:model.live="name" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-2xl focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-500" placeholder="e.g. Tutorial" />
                                    @error('name') <span class="text-xs text-rose-500 font-semibold">{{ $message }}</span> @enderror
                                </div>

                                <div>
                                    <label class="text-xs font-bold text-slate-400 dark:text-slate-500 block mb-2 uppercase tracking-wider">Tag Slug</label>
                                    <input type="text" wire:model="slug" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-2xl focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-500" placeholder="e.g. tutorial" />
                                    @error('slug') <span class="text-xs text-rose-500 font-semibold">{{ $message }}</span> @enderror
                                </div>

                                {{-- ─── Translations Section ──────────────────────────────────────────────── --}}
                                @if($activeLanguages->isNotEmpty() && $tagId)
                                        ...
                                        @if($tlLangId > 0)
                                            <div class="space-y-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
                                                ...
                                                <div class="flex gap-2 pt-1">
                                                    <button type="button" wire:click="aiTlTag({{ $tagId }})"
                                                            wire:loading.attr="disabled" wire:target="aiTlTag({{ $tagId }})"
                                                        ...
                                                        <span wire:loading.remove wire:target="aiTlTag({{ $tagId }})">
                                                        ...
                                                        <span wire:loading wire:target="aiTlTag({{ $tagId }})" class="inline-flex">
                                                            <svg class="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
                                                        </span>
                                                        AI Translate All
                                                    </button>
                                                    <button type="button" wire:click="saveTlTag({{ $tagId }})"
