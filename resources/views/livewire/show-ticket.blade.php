            ...
            <div class="grid gap-6 lg:grid-cols-3">
                ...
                <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="flex items-center justify-between pb-4 border-b border-slate-100 mb-5">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-400">@label('ticket.original_request', 'Original Request')</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="space-y-6">
                        <h3 class="font-bold text-slate-800 text-lg flex items-center gap-2">
                            ...
                            @label('ticket.replies_heading', 'Replies & Updates')
                        </h3>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                @if ($isSupport)
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded-md bg-indigo-100 text-indigo-700 text-[10px] font-bold tracking-wider uppercase">
                                                        @label('ticket.support_agent', 'Support Agent')
                                                    </span>
                                                @endif
                                            </div>
                        ...
                        @empty
                            <div class="border border-dashed border-slate-200 rounded-2xl p-8 text-center text-slate-400 text-sm">
                                @label('ticket.no_replies', 'No replies recorded yet. You\'ll receive email updates as our team reviews your request.')
                            </div>
                        @endforelse
                    </div>
                    ...
                    @if ($ticket->status->value !== 'closed')
                        <div class="bg-white border border-slate-200/70 shadow-sm rounded-2xl p-6 sm:p-8">
                            <h3 class="font-bold text-slate-800 text-lg">@label('ticket.post_reply', 'Post a Reply')</h3>
                            <form wire:submit="reply" class="mt-4 space-y-4">
                                <div>
                                    <textarea wire:model="body" rows="5"
                                              ...
                                              placeholder="@label('ticket.reply_placeholder', 'Type your reply here...')" required></textarea>
                                    <x-input-error :messages="$errors->get('body')" class="mt-2 text-xs" />
                                </div>

                                <!-- File Upload -->
                                <div>
                                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">@label('ticket.upload_optional', 'Upload Files (optional)')</label>
                                    ...
                                    @if (!empty($attachments))
                                        <div class="mt-2 text-xs text-slate-500">
                                            @label('ticket.files_selected_label', 'Files selected:') {{ count($attachments) }}
                                        </div>
                                    @endif
                                ...
                                <div class="flex justify-end pt-2 border-t border-slate-100">
                                    <x-primary-button wire:loading.attr="disabled" class="rounded-xl px-5 py-2.5">
                                        <span wire:loading.remove wire:target="reply">@label('ticket.send_message', 'Send Message')</span>
                    ...
                    @else
                        <div class="rounded-2xl border border-slate-200/80 bg-slate-50 p-5 text-center text-sm text-slate-500 shadow-sm">
                            <span class="inline-flex items-center gap-1.5 font-semibold text-slate-700">
                                ...
                                @label('ticket.closed_heading', 'This ticket is closed')
                            </span>
                            <p class="mt-1 text-xs">@label('ticket.closed_message', 'Replies are disabled. If you have further issues, please submit a new ticket.')</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="space-y-6">
                    <div class="bg-white border border-slate-200/70 shadow-sm rounded-2xl p-6 space-y-5">
                        <h3 class="font-bold text-slate-800 text-base pb-3 border-b border-slate-100">@label('ticket.info_heading', 'Ticket Information')</h3>
                        
                        <div class="space-y-4">
                            <div>
                                <label class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">@label('ticket.assigned_agent', 'Assigned Agent')</label>
                                <div class="mt-1 flex items-center gap-2">
                                    ...
                                    <span class="text-sm font-semibold text-slate-700">
                                        ...
                                        @else
                                            @label('ticket.unassigned', 'Unassigned')
                                        @endif
                                    </span>
                                </div>
                            </div>
                            ...
                            <div>
                                <label class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">@label('ticket.last_activity', 'Last Activity')</label>
                            ...
                            <div>
                                <label class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">@label('ticket.reply_via_email', 'Reply via Email')</label>
                                ...
                                <p class="text-[10px] text-slate-400 mt-1">@label('ticket.reply_via_email_note', 'Send mail to this address to post replies directly.')</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-tr from-slate-900 to-indigo-950 text-slate-100 shadow-lg shadow-indigo-900/10 rounded-2xl p-6 relative overflow-hidden">
                        ...
                        <h3 class="font-bold text-white text-base">@label('ticket.secure_link', 'Secure Public Link')</h3>
                        <p class="text-xs text-slate-300 mt-1 leading-relaxed">@label('ticket.secure_link_message', 'Share this secret URL to access this ticket without needing to log in.')</p>
                        ...
                        <div class="mt-4" x-data="{ copied: false, url: '{{ $ticket->publicUrl() }}' }">
                            ...
                            <button @click="navigator.clipboard.writeText(url); copied = true; setTimeout(() => copied = false, 2000)"
                                ...
                                <span x-show="!copied">@label('ticket.copy_link', 'Copy Link')</span>
                                <span x-show="copied" class="flex items-center gap-1">
                                    <svg class="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                    @label('ticket.copied', 'Copied!')
                                </span>
                            </button>
                        </div>
                    </div>
