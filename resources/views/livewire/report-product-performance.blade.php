<div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm flex flex-col space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <h4 class="text-sm font-bold text-slate-900 uppercase tracking-wider font-sans">Product Sales Performance</h4>
            <p class="text-xs text-slate-400 mt-0.5 font-sans">View your top-performing and lowest-selling items.</p>
        </div>
    ...
    <div class="flex border-b border-slate-100 p-0.5 bg-slate-50/80 rounded-2xl">
        <button type="button" 
                wire:click="setPerformanceMode('highest')"
                class="flex-1 py-2 text-xs font-bold rounded-xl transition duration-150 flex items-center justify-center gap-1.5 {{ $performanceMode === 'highest' ? 'bg-white text-indigo-600 shadow-sm border border-slate-200/40' : 'text-slate-500 hover:text-slate-700' }}">
        ...
        <button type="button" 
                ...
                class="flex-1 py-2 text-xs font-bold rounded-xl transition duration-150 flex items-center justify-center gap-1.5 {{ $performanceMode === 'lowest' ? 'bg-white text-rose-600 shadow-sm border border-slate-200/40' : 'text-slate-500 hover:text-slate-700' }}">
                ...
                <tbody class="divide-y divide-slate-100 font-sans">
                    @foreach($products as $prod)
                        <tr class="hover:bg-slate-50/40 transition duration-100">
                            <td class="py-3.5 font-bold text-slate-800 pr-4 max-w-[200px] truncate" title="{{ $prod->item_name }}">
                                @if($prod->product_id)
                                    <a href="{{ route('admin.ecommerce.product-edit', $prod->product_id) }}" class="hover:underline text-slate-800">{{ $prod->item_name }}</a>
                                @else
                                    {{ $prod->item_name }}
                                @endif
                            </td>
                            <td class="py-3.5 text-center font-extrabold text-slate-700">{{ number_format($prod->total_qty) }}</td>
                            <td class="py-3.5 text-right font-extrabold text-indigo-600">${{ number_format($prod->total_revenue, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
