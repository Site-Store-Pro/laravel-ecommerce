...
<head>
    ...
    <title>{{ !empty($exception->getMessage()) ? $exception->getMessage() : 'Access Forbidden' }} | {{ config('app.name', 'Store') }}</title>
...
<body class="min-h-screen flex items-center justify-center p-4 sm:p-6 bg-slate-50 selection:bg-indigo-500 selection:text-white">
    ...
    <div class="relative z-10 w-full max-w-lg bg-white border border-slate-100 rounded-3xl p-8 sm:p-10 shadow-xl shadow-slate-200/50 text-center space-y-6">
        <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-amber-50 border border-amber-100 text-amber-600 shadow-inner">
        ...
        <div class="space-y-2">
            <span class="inline-block px-3 py-1 bg-amber-100 text-amber-800 text-[11px] font-black uppercase tracking-wider rounded-full">
                403 &bull; {{ str_contains(strtolower($exception->getMessage() ?? ''), 'expired') ? 'Link Expired' : 'Access Restricted' }}
            </span>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
                {{ !empty($exception->getMessage()) ? $exception->getMessage() : 'Access Restricted' }}
            </h1>
            <p class="text-sm text-slate-500 max-w-sm mx-auto leading-relaxed">
                @if(str_contains(strtolower($exception->getMessage() ?? ''), 'expired'))
                    This content or download access link has expired. If you recently purchased this item, please log in to your account or contact customer support for a refreshed link.
                @else
                    You do not have permission to view or download this content. Please verify your credentials or contact support if you believe this is an error.
                @endif
            </p>
        </div>
