...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth bg-slate-50 text-slate-800">
    <head>
        ...
        <title>{{ $category->name }} | {{ config('app.name', 'Support Desk') }}</title>
        <meta name="description" content="Browse all articles and posts in the {{ $category->name }} category.">
                ...
                <div class="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
                    ...
                    <div class="text-center max-w-3xl mx-auto mb-16 space-y-4">
                        ...
                        <p class="text-base text-slate-500 leading-relaxed font-medium">
                            Browse all content and updates published under the <span class="text-indigo-600 font-semibold">{{ $category->name }}</span> tag folder.
                        </p>
                    </div>
                                ...
                                <article class="group bg-white/95 backdrop-blur-md border border-slate-100/80 rounded-3xl overflow-hidden shadow-xl shadow-slate-100/40 hover:shadow-2xl hover:shadow-slate-200/50 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between">
                                    <div>
                                        ...
                                        <a href="{{ route('page.show', $post->slug) }}" class="block relative aspect-[16/10] overflow-hidden bg-slate-100">
                                            ...
                                            @else
                                                <div class="w-full h-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center opacity-85">
                                        ...
                                        <div class="p-6 md:p-8 space-y-4">
                                            <div class="flex items-center gap-2.5 flex-wrap">
                                                @foreach($post->tags as $tag)
                                                    <span class="text-2xs font-extrabold bg-slate-50 text-slate-500 px-2 py-0.5 rounded-md uppercase tracking-wider">#{{ $tag->name }}</span>
                                                @endforeach
                                            </div>

                                            <h3 class="text-xl font-bold text-slate-900 leading-tight group-hover:text-indigo-600 transition-colors">
                    ...
                    @else
                        <div class="bg-white border border-slate-100 rounded-3xl p-12 text-center max-w-xl mx-auto shadow-sm space-y-4">
                            ...
                            <p class="text-sm text-slate-400 font-medium">There are currently no active or published posts in this category folder.</p>
                        </div>
                    @endif
