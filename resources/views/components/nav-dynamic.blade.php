@php
    ...
    $stickyRule = $menu->sticky ? 'position: sticky; top: 0; z-index: 50;' : '';
...
<style id="top-nav-{{ $menu->slug }}-styles">
{!! CssMinifierService::minify($navRawCss) !!}
</style>
...
<header id="top-nav-{{ $menu->slug }}"
    ...
    <div x-show="mobileOpen"
         ...
         class="lg:hidden px-4 pb-4 pt-2 border-t border-slate-100 space-y-1"
         style="background: var(--nav-mobile-bg, #ffffff)">
        @foreach($items as $item)
            @if(!$item->isVisibleFor($context['user'])) @continue @endif
