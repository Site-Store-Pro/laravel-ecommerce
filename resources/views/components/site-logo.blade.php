@php
    ...
    $showLogo = \App\Models\CmsSetting::isEnabled('header_show_logo', true);
    $showTitle = \App\Models\CmsSetting::isEnabled('header_show_site_title', true);
@endphp

@if($showLogo || $showTitle)
<a href="/" class="inline-flex items-center gap-1 group w-auto max-w-max min-w-0 shrink-0 my-auto py-0.5" title="{{ $siteName }}">
    @if($showLogo)
        @if($logo['type'] === 'url')
