...
`
    ...
    const initDependencies = (config) => {
        const scripts = [
            'https://cdn.jsdelivr.net/npm/ace-builds@1.35.0/src-noconflict/ace.js',
            'https://cdnjs.cloudflare.com/ajax/libs/js-beautify/1.15.1/beautify-html.min.js'
        ];
        
        if (config.autocomplete) {
            scripts.push('https://cdn.jsdelivr.net/npm/ace-builds@1.35.0/src-noconflict/ext-language_tools.js');
        }
    ...
    const mainPlugin = function(editor) {
        ...
        const setAceOptions = () => {
            ...
            if(Config.autocomplete){
                try {
                    ...
                    console.error("Failed to load Ace autocomplete extension:", e);
                }
            }
        ...
        const showModal = () => {
            if (typeof ace === 'undefined' || typeof html_beautify === 'undefined') {
                alert('Source editor dependencies are still loading. Please try again in a few seconds.');
                return;
            }
            if(!modal){
                // Build Modal
        ...
        const loadAceSession = () => {
            ...
            try {
                ...
                if (range) {
                    markerEl = editor.getDoc().createElement('span');
                    markerEl.id = 'supercode-temp-cursor-marker';
                    markerEl.style.display = 'none';
                    markerEl.appendChild(editor.getDoc().createTextNode('\u200B'));
                    range.insertNode(markerEl);
                }
            } catch (e) {
                console.error("Failed to insert temp cursor marker:", e);
            }
            ...
            let formattedContent = getSourceCode(rawContent);
            let markerIndex = -1;
            const markerRegex = /<span[^>]*id=["']supercode-temp-cursor-marker["'][^>]*>[\s\S]*?<\/span>/gi;
            
            let firstMatch = markerRegex.exec(formattedContent);
            if (firstMatch) {
                markerIndex = firstMatch.index;
            }

          
            formattedContent = formattedContent.replace(markerRegex, '');

            if(!session){
                session = ace.createEditSession(formattedContent, `ace/mode/${Config.language}`);
            ...
            if (markerIndex !== -1) {
                const pos = session.getDocument().indexToPosition(markerIndex, 0);
                aceEditor.moveCursorToPosition(pos);
                aceEditor.scrollToLine(pos.row, true, true, function() {});
            } else {
