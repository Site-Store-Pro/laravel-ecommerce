...
> Combined reference for the Laravel E-Commerce, CMS & Helpdesk platform.  
> Source: `README.md` + `README-docs.md` â€” merged into a single authoritative document.


## Table of Contents

1. [Platform Overview & Key Modules](#platform-overview--key-modules)
2. [Technology Stack](#technology-stack)
3. [Installation & Local Setup](#installation--local-setup)
4. [Directory Structure & Blade View Architecture](#directory-structure--blade-view-architecture)
5. [User Roles & Permissions](#user-roles--permissions)
6. [Demo Seeding & Default Credentials](#demo-seeding--default-credentials)
7. [E-Commerce: Storefront & Shopping](#e-commerce-storefront--shopping)
8. [E-Commerce: Administration](#e-commerce-administration)
9. [Product Reviews & Ratings](#product-reviews--ratings)
10. [Image & Asset Storage](#image--asset-storage)
11. [Discount & Pricing Engine](#discount--pricing-engine)
12. [Shipping, Tax & Surcharges](#shipping-tax--surcharges)
13. [Payment Gateways & Subscriptions](#payment-gateways--subscriptions)
14. [Digital Downloads â€” Product Variants](#digital-downloads--product-variants)
15. [CMS Downloads Manager](#cms-downloads-manager)
16. [Inventory & Warehouse Management](#inventory--warehouse-management)
17. [Bulk Product & Variant Import](#bulk-product--variant-import)
18. [Product COPY / Duplication](#product-copy--duplication)
19. [CMS System: Pages, Posts & Content](#cms-system-pages-posts--content)
20. [CMS Page Access Gating](#cms-page-access-gating)
21. [Post-Order Completion Redirect](#post-order-completion-redirect)
22. [Events Calendar Plugin](#events-calendar-plugin)
23. [Header & Footer Builder & CSS Manager](#header--footer-builder--css-manager)
24. [Global Settings & Theme Customization](#global-settings--theme-customization)
25. [Advanced Shop Search Filtering](#advanced-shop-search-filtering)
26. [Plugin System](#plugin-system)
27. [Shortcode System & Parsing Pipelines](#shortcode-system--parsing-pipelines)
28. [Reusable List Menus](#reusable-list-menus)
29. [Search System & Collated Index Engine](#search-system--collated-index-engine)
30. [Dynamic Email Notifications](#dynamic-email-notifications)
31. [Multilingual & Translation System](#multilingual--translation-system)
32. [Knowledge Base](#knowledge-base)
33. [Helpdesk & Ticketing](#helpdesk--ticketing)
34. [Dynamic Blade & Livewire Content Parser](#dynamic-blade--livewire-content-parser)
35. [Custom Hashing & Login Security](#custom-hashing--login-security)
36. [Inbound Email Webhook Ingestion](#inbound-email-webhook-ingestion)
37. [API & Webhook Endpoints (Routing Directory)](#api--webhook-endpoints-routing-directory)
38. [Timezone Configuration](#timezone-configuration)
39. [Merchant Location, Currency & VAT Settings](#merchant-location-currency--vat-settings)
40. [Dynamic Product Layout Selector](#dynamic-product-layout-selector)
41. [Production Checklist & Upgrading](#production-checklist--upgrading)
42. [Developer Reference](#developer-reference)

---


## Platform Overview & Key Modules

A comprehensive, enterprise-ready e-commerce, CMS, and helpdesk platform built on Laravel 13, Livewire 3, Tailwind CSS, and Alpine.js. This platform combines a dynamic, high-performance storefront, a full-featured CMS / site-builder, and a complete e-commerce and support administration workspace.

## ðŸš€ Key Modules & Architecture
...
* **Product Edit Page (`/admin/ecommerce/products/{id}/edit`)**:
  * **Anchor Quick-Nav Bar**: Jump links at the top â€” _Product Details_, _Prices & Variants_, _Advanced Settings_, and a **"View Product Page"** link to view the product live on the public storefront.
  ...
  * **Prices & Variants**: Full variant management with pricing, inventory, download config, and image set management:
    ...
    * **Charge Sales Tax / VAT (`charge_tax`)**: A per-variant toggle (default: **ON / Taxable**) that controls whether this variant is included in the taxable subtotal during checkout. When toggled off, the variant's line-item price is excluded from tax calculations entirely â€” only variants with `charge_tax = 1` contribute to the taxable base. The toggle is displayed in both the variant edit and create forms and also appears as a **Tax** column (green â— Taxable / grey â— Exempt badge) in the variants list table for quick visual reference.
    * **Payment Processor IDs**: Each variant has a collapsible **Payment Processor IDs** section in both the edit and create forms. This panel stores the gateway-specific price/plan identifiers needed to support **subscription (recurring) billing**. Fields include:
      ...
      | `create_new_stripe_product` | Toggle â€” when **ON**, a new Stripe Product + recurring Price is created automatically at checkout; existing price ID fields are ignored |
      | `stripe_billing_interval` | Billing frequency when creating on-the-fly: `month` (Monthly), `year` (Yearly), or `week` (Weekly) |
      | `stripe_trial_enabled` | Toggle â€” enables a free trial period for new subscribers |
      | `stripe_trial_days` | Number of free trial days (e.g. `14`) â€” only active when `stripe_trial_enabled = 1` |
...
| **Direct External URL** | `image_url_source = 1` â€” path rendered as-is, no Storage lookup |
...
#### Pipeline A â€” `ProcessShortcodes` Middleware (list menus only)
...
3. **Embedded Plugin/Download Shortcodes in Items**: After the `[page/product/category/brand]` pass, `ListMenuRenderer::parseItemContent()` calls `ShortcodeProcessor::process()` â€” so any `[download:N]` or `[plugin:slug]` shortcodes _embedded inside a list menu item_ are also resolved here.
...
#### Pipeline B â€” `ContentParserService::parse()` (CMS page content & product descriptions)
...
2. **`ShortcodeProcessor::process()`**: Runs immediately after Blade compilation in two ordered passes:
   - **Pass 1 â€” `[download:N]`**: `processDownloads()` runs first, resolving all CMS download shortcodes to their HTML output (image, video player, audio player, or styled link).
   - **Pass 2 â€” `[plugin:slug]`**: Plugin shortcodes are expanded after downloads to prevent any ambiguity between numeric IDs and plugin slugs.

### 5. Shortcode Support Matrix

The table below shows which shortcode types work in each content field and which pipeline delivers them. Both pipelines run on every public page request â€” Pipeline A over the final HTML, Pipeline B at content-parse time â€” so fields that go through Pipeline B automatically also benefit from Pipeline A.

| Content Field | `[list:N]`<br>`[page:N]`<br>`[product:N]`<br>`[category:N]`<br>`[brand:N]` | `[download:N]` | `[plugin:slug]` | Blade `@auth`<br>`{{ }}` |
|---|:---:|:---:|:---:|:---:|
| **CMS Page â€” main body** | âœ… Pipeline A | âœ… Pipeline B | âœ… Pipeline B | âœ… Pipeline B |
| **CMS Page â€” left sidebar** | âœ… Pipeline A | âœ… Pipeline B | âœ… Pipeline B | âœ… Pipeline B |
| **CMS Page â€” right sidebar** | âœ… Pipeline A | âœ… Pipeline B | âœ… Pipeline B | âœ… Pipeline B |
| **Product short description** | âœ… Pipeline A | âœ… Pipeline B | âœ… Pipeline B | âœ… Pipeline B |
| **Product long description** | âœ… Pipeline A | âœ… Pipeline B | âœ… Pipeline B | âœ… Pipeline B |
| **List menu items** | âœ… Pipeline A | âœ… Pipeline AÂ¹ | âœ… Pipeline AÂ¹ | âŒ |
| **Site footer / header layouts** | âœ… Pipeline A | âŒ | âŒ | âŒ |

> Â¹ `[download:N]` and `[plugin:slug]` shortcodes embedded **inside a list menu item** are resolved because `ListMenuRenderer::parseItemContent()` calls `ShortcodeProcessor::process()` internally as part of Pipeline A.

#### How to read this table

- **Pipeline A** (`ProcessShortcodes` middleware) â€” scans the **final rendered HTML** of every public web response. Works on every field whose output reaches the browser, including sidebars, footers, and headers. Resolves `[list:N]`, `[page:N]`, `[product:N]`, `[category:N]`, and `[brand:N]`.
- **Pipeline B** (`ContentParserService::parse()`) â€” called explicitly per field, before the field's content is inserted into the page template. Resolves Blade directives, `[download:N]`, and `[plugin:slug]`. Only fields that use a `parsed_*` model accessor go through this pipeline.
...
* **Dynamic Tax Label**: The tax line on the order review, checkout success, and email receipts reads *Sales Tax* (US), *GST/HST* (Canada), or *VAT* (all other countries) â€” determined automatically from the buyer's shipping country.
* **Selective Taxable Subtotal**: Tax is calculated only on the portion of the order that is taxable â€” not the entire cart subtotal. See [Per-Item Taxability](#3-per-item-taxability-charge_tax) below.
...
```
taxableSubtotal = Î£ (item.final_price Ã— item.qty)  for items where item_taxable = 1
```

The `TaxCalculationService` receives only this taxable portion â€” not the full cart subtotal. Non-taxable items are included in the order total but contribute **zero** to the tax base.
...
* **Variants list table**: A **Tax** column shows a color-coded badge (green â— Taxable / grey â— Exempt) for every variant at a glance without entering edit mode.
...
1. If `image_url_source = 1` â†’ return the path as a direct URL immediately.
2. If `cdn_url` is set â†’ prefix the path with the CDN domain.
3. If `image_s3 = 1` â†’ resolve via the S3 disk (global or custom credentials).
4. Otherwise â†’ resolve via the local `public` disk.
...
| **CMS Navigation** | Admin nav â†’ **CMS** dropdown â†’ **Downloads** |
| **CMS Sidebar** | Left sidebar of any CMS admin page â†’ **Downloads** link |
| **Settings** | `/admin/settings` â†’ **CMS Downloads** section |

---

### 2. How a Download Is Stored

Each CMS Download record is stored in the `cms_downloads` table. A single record captures everything needed to resolve, serve, and display a file â€” regardless of where the file is physically located.
...
| `is_active` | `boolean` | Inactive records render as an HTML comment â€” never a 404 |
...
#### Source Type 1 â€” Direct URL
The `cdn_url` field contains a full public URL (e.g., a CloudFront or Cloudflare R2 URL). The controller issues a `302` redirect to that URL. No server-side streaming occurs â€” the CDN handles bandwidth directly.

#### Source Type 2 â€” Env S3 (Global Credentials)
...
#### Source Type 3 â€” Custom S3 (Per-Record Credentials)
...
> **Security note:** Pre-signed S3 URLs are time-limited (default: 1 hour). They grant temporary, read-only, unauthenticated access to the specific S3 object â€” no AWS credentials are ever exposed to the browser.
...
The `label` parameter is optional. When omitted, the download's saved `link_label` field is used as the link text. When provided, the inline label overrides the database default for that specific embed only â€” the record itself is unchanged.
...
### 5. Shortcode Rendering â€” Four Output Modes
...
#### Mode A â€” Inline Image
**Triggered for extensions:** `png Â· jpg Â· jpeg Â· jpe Â· gif Â· bmp Â· webp Â· svg Â· svgz Â· tif Â· tiff`
...
#### Mode B â€” Video.js Player
**Triggered for extensions:** `mp4 Â· webm Â· mov Â· qt`
...
```html
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/â€¦/video-js.min.css">
<script src="https://cdnjs.cloudflare.com/â€¦/video.min.js"></script>
<div class="cms-video-display-container">
    <video id="cms-media-12-84729"
           class="video-js vjs-fluid vjs-default-skin"
           controls preload="auto" width="100%" height="100%"
           poster="https://â€¦/poster.jpg">
...
#### Mode C ï¿½ Video.js Audio Player
**Triggered for extensions:** `mp3`
**Condition:** `isAudio()` returns `true` AND `force_download` is `false`

Renders an `<audio>` element wrapped in a Video.js player (same CSS/JS assets as video mode ï¿½ deduplication applies). Poster art is fully supported and recommended; it displays as a thumbnail above the audio controls ï¿½ ideal for album art or episode cover images:
```html
...
#### Mode D ï¿½ Styled Download Link
**Triggered for:** all other file types (PDF, ZIP, DOCX, EXE, CSV, etc.) ï¿½ and any media/image type when `force_download` is `true`
...
When an icon position is configured, `ShortcodeProcessor` emits a scoped `<style>` block keyed to the download ID alongside the link HTML ï¿½ no global CSS class conflicts are possible between multiple different download shortcodes on the same page:
```html
...
The active icon pack is a **global site setting** configured at `/admin/settings` â†’ **CMS Downloads** section. Three packs are available:

| Pack | Setting Value | CSS Class | Description |
|---|---|---|---|
| **Vivid** *(default)* | `vivid` | `fiv-viv` | Bold, colourful filled icons â€” best on light backgrounds |
| **Classic** | `classic` | `fiv-cla` | Clean monochrome style â€” timeless and professional |
| **Square** | `square` | `fiv-sqo` | Square outline style â€” modern and minimal |
...
- **Admin layout** (`layouts/app.blade.php`): All three pack CSS files are loaded simultaneously so the Settings page live-preview works instantly when toggling between packs â€” no page reload required.
- **Public layout** (`layouts/public.blade.php`): Only the active pack's single CSS file is loaded, keeping public page weight lean.

**Supported file extension icons** include: `pdf Â· doc Â· docx Â· xls Â· xlsx Â· ppt Â· pptx Â· zip Â· rar Â· exe Â· mp3 Â· mp4 Â· jpg Â· png Â· gif Â· svg Â· txt Â· odt` and many more â€” covering the full file-icon-vectors icon catalogue. Any unrecognised extension renders without an icon gracefully, with no errors.
...
**Basic Info**
- `Internal Name` â€” admin-only label used in search and the shortcode generator drawer; not shown to visitors
- `Link Label` â€” the public-facing link text used when no `label=` override is supplied in the shortcode
- Shortcode preview badge â€” displays the exact `[download:{id} label="..."]` string to copy into any TinyMCE editor

**File Source** â€” mutually exclusive tabbed panel:
...
**Display Options**
- **File Type Icon** â€” 5-button radio position picker (None / Left / Right / Top / Bottom) with directional arrow icons and a live `fiv-icon-*` preview rendered below the picker as soon as a file source is configured
- **Open in New Tab** â€” toggle that adds `target="_blank" rel="noopener noreferrer"` to the generated link
- **Force Download** â€” forces a browser `Content-Disposition: attachment` prompt on all file types, bypassing inline image/video/audio rendering

**Lifecycle Options**
- **Active** toggle â€” when off, the shortcode renders a silent HTML comment (`<!-- [download-inactive: N] -->`) rather than a broken link or 404
- **Expiry Date** â€” optional `datetime-local` picker; once the date passes the link is silently suppressed on the public side

**Advanced**
- **Custom CSS** â€” raw CSS entered here is scoped to the download ID via `<style id="cms-dl-style-{id}">` and injected inline with the rendered output
...
1. **Pass 1 â€” `[download:N]` shortcodes** â€” handled first by `processDownloads()`. The regex `/\[download:(\d+)([^\]]*)\]/i` captures the numeric ID and any trailing `label="..."` parameter before the plugin pass runs.
2. **Pass 2 â€” `[plugin:slug]` shortcodes** â€” the plugin system runs after downloads are fully resolved.
...
| **Access control** | Optionally public â€” no login required | Requires valid order UUID token |
...
1. Navigate to **Admin â†’ Settings** (`/admin/settings`).
...
Navigate to **Admin â†’ Settings** (`/admin/settings`). A **Timezone** dropdown selector is displayed in the General Settings panel. Select any valid PHP timezone and click **Save Settings**.
...
* Timezone takes effect immediately on next request after saving â€” no restart or cache flush required.
...
* Defaults to `$`.
* Common examples: `Â£` (British Pound), `â‚¬` (Euro), `A$` (Australian Dollar), `C$` (Canadian Dollar), `Â¥` (Japanese Yen).
* The symbol is always **prefix-positioned** (e.g. `Â£19.99`).
...
In most countries outside the US and Canada (e.g. UK, EU member states, Australia, New Zealand), prices in a shop are displayed **including VAT**. When a customer in a VAT country shops domestically, the price they see and pay already contains the tax. When a customer from a non-VAT country (US or Canada) makes a cross-border purchase, they should not be charged VAT â€” so it must be stripped from the displayed price and totals.
...
* No price data is corrupted â€” VAT is a pure display layer.
...
* All prices on the catalog, product detail, and cart pages are shown **inclusive of VAT** (net price Ã— 1.2 for 20% VAT).
* The order review page shows a VAT breakdown line: **"Includes VAT Â£X.XX"** â€” informational, not additive.
...
* No VAT line appears on the order review â€” the customer pays the net price only.
* Email totals reflect the stripped (ex-VAT) amounts.

> **Example**: A product stored at Â£16.67 ex-VAT. UK buyer sees Â£20.00 (incl. 20% VAT). US buyer sees $16.67 (VAT stripped, shown in configured currency symbol).
...
| `100+` | **Custom Gateway** | Optional / manual | `payment-processors/my-gateway/` (see Â§8) |

### 2. Architecture Overview

```
app/Services/Payments/
  Contracts/PaymentProcessorInterface.php   â† Interface all drivers implement
  PaymentResult.php                         â† Uniform result DTO
  PaymentProcessorManager.php               â† Resolves active driver + randomize logic
  Processors/
    TestProcessor.php                       â† Built-in test simulator (always active)
    StripeProcessor.php                     â† Built-in Stripe driver (always registered)
    PaddleProcessor.php                     â† Built-in Paddle driver (always registered)
    PayPalProcessor.php                     â† Built-in PayPal driver (always registered)

config/payment_processors.php              â† Registry: maps processor_id â†’ class

payment-processors/
  stripe/
    StripeProcessor.php                     â† Extension TEMPLATE (not active by default)
    README.md                               â† Stripe setup + extension guide
  paddle/
    PaddleProcessor.php                     â† Extension TEMPLATE (not active by default)
    README.md                               â† Paddle setup + extension guide
  paypal/
    PayPalProcessor.php                     â† Extension TEMPLATE (not active by default)
    README.md                               â† PayPal setup + extension guide
  example-gateway/
    ExampleGatewayProcessor.php             â† New processor TEMPLATE (not active)
    README.md                               â† Step-by-step guide for new gateways
```
...
The platform **auto-detects** these files â€” no changes to `config/payment_processors.php` needed:
...
{
    // Override only the methods you need â€” everything else is inherited.
...
**Step 2** â€” Add credentials to `.env`:
```ini
# Production
...
**Step 3** â€” In Admin â†’ Checkout â†’ Processors, set **Stripe** as Primary and toggle Production on/off.
...
**Step 3** â€” In Admin â†’ Checkout â†’ Processors, set **Paddle** as Primary and toggle Production on/off.
...
**Step 1** â€” Add credentials to `.env`:
```ini
# Production
...
**Step 2** â€” In Admin â†’ Checkout â†’ Processors, set **PayPal** as Primary and toggle Production on/off.
...
The sandbox/production flag is read at runtime from the database â€” no code changes needed to switch environments.
...
1. **Randomize OFF** â†’ use `primary_processor` from `order_checkout_options`.
2. **Randomize ON** â†’ randomly selects from all non-zero configured slots (primary / secondary / tertiary) registered in the registry. Falls back to primary if none are registered.
3. **Fallback** â†’ Test Processor (ID 0).

**Checkout display behavior:**
* Only Test Processor active â†’ mock payment card shown.
* Real processor active (Stripe or Paddle) â†’ that processor's JS widget shown.
* Randomize ON â†’ a random real processor's widget is selected per page load.
...
4. Insert a row into `order_processors` with your chosen `processor_id` (use 100 or higher â€” IDs 0 to 99 are reserved)
...
7. In Admin â†’ Checkout â†’ Processors, set as Primary

> The `payment-processors/` directory is outside `app/` by design â€” its contents are never overwritten by platform updates.

### 9. Checkout Flow (Two-Step)

1. **`preparePayment()`** (Livewire server call): Scans cart for subscription variants. Then:
   - **Subscription cart (Stripe)**: Calls `StripeProcessor::createSubscription()` â€” resolves or creates a Stripe Customer + Price, creates the Subscription, returns a `client_secret` for browser confirmation.
   ...
   - **PayPal** (regular): Calls `PayPalProcessor::createOrder()` â€” creates a v2 Order via PayPal Orders API and returns the unique PayPal Order ID and Client ID.
...
Register in Stripe Dashboard: **Developers â†’ Webhooks â†’ Add endpoint**
```
...
| Event | Action |
|---|---|
| `payment_intent.succeeded` | Finds order by `authorization_code`, ensures status â‰¥ 1 (Open) |
...
| `customer.subscription.created` | Logged â€” extend for subscription entitlement grants |
| `customer.subscription.updated` | Logged â€” extend for plan changes, downgrades |
| `customer.subscription.deleted` | Logged â€” extend for access revocation on cancellation |
| `invoice.payment_succeeded` | Logged â€” extend for recurring renewal confirmations |
| `invoice.payment_failed` | Logged â€” extend for dunning / payment retry logic |

#### Paddle Webhook â€” `POST /webhooks/paddle`

Register in Paddle Dashboard: **Developer Tools â†’ Notifications â†’ New Destination**
```
...
| Event | Action |
|---|---|
| `transaction.completed` | Finds order by `authorization_code`, ensures status â‰¥ 1 (Open) |
...
| `subscription.created` | Logged â€” extend for subscription entitlement grants |
| `subscription.updated` | Logged â€” extend for plan changes and quantity updates |
| `subscription.canceled` | Logged â€” extend for access revocation |
| `subscription.payment_failed` | Logged â€” extend for dunning logic |
...
The platform fully supports **subscription (recurring) billing** through Stripe and Paddle. Subscriptions are configured at the **product variant level** â€” any variant that has at least one payment processor price ID set is treated as a subscription variant.
...
Navigate to **Admin â†’ Products â†’ Edit Product â†’ Prices &amp; Variants** and open or create a variant. The collapsible **Payment Processor IDs** section contains all subscription configuration fields:

**Paddle:**
- Enter the `pri_sandbox_â€¦` Price ID for test mode and/or the `pri_â€¦` Price ID for live mode.
...
When using **existing Stripe Price IDs**, you may still configure the billing interval and trial â€” these values are passed as metadata context (useful for display or audit purposes; the actual interval is controlled by the pre-created Stripe Price).
...
- Adding a subscription item when the cart already contains a regular item â†’ **blocked with an error message**.
- Adding a regular item when the cart already contains a subscription item â†’ **blocked with an error message**.
...
```
Cart scan â†’ subscription variant found?
  YES â†’ Stripe: createSubscription()  |  Paddle: createTransaction(price_id=...)
  NO  â†’ Stripe: createPaymentIntent() |  Paddle: createTransaction(custom amount)
```
...
2. Passes the Price ID to `createTransaction()` â€” Paddle Billing handles the subscription lifecycle from there.
...
* If a fee is configured, the fee is displayed next to the label (e.g. `(+Â£5.00)`).
...
* Remaining download count (max âˆ’ used).
...
- [ ] In Admin â†’ Checkout â†’ Processors, set the desired processor as **Primary** and toggle **Production ON**.
...
The plugin system is a fully dynamic, database-driven architecture that enables **display elements** (Swiper slideshows, galleries) and **shipping providers** (FedEx, UPS) to be registered, configured, and embedded into any CMS page or product description â€” without code changes or server restarts.
...
Three tables power the system â€” created by `2026_07_18_100000_create_plugins_tables.php`.

### `plugins` â€” Plugin Registry
...
### `plugin_options` â€” Settings Form Schema

Defines the configuration form for each plugin. Fully data-driven â€” no hardcoded admin forms.

| Column | Type | Description |
|---|---|---|
| `id` | bigint PK | |
| `plugin_id` | unsignedBigInt | FK â†’ `plugins.id` |
...
### `plugin_settings` â€” Saved Values
...
app/
  Plugins/
    Contracts/
      DisplayPlugin.php          â† Interface all display plugins must implement
      ShippingPlugin.php         â† Interface all shipping plugins must implement
    Support/
      PluginManager.php          â† Singleton: discovers, registers, renders plugins
      ShortcodeProcessor.php     â† Parses [plugin:slug] shortcodes from content
      ShippingContext.php        â† Readonly DTO passed to shipping plugin getRates()
    Display/
      SlideshowPlugin.php        â† Built-in: Swiper.js slideshow display plugin
      FeaturedItemsPlugin.php    â† Built-in: Featured products grid/list/slider display
    Shipping/
      FedExPlugin.php            â† Built-in: FedEx REST API v1 shipping plugin
  Http/Controllers/
    PluginApiController.php      â† JSON endpoint: list active display plugins
  Livewire/
    AdminPlugins.php             â† Admin panel Livewire component
  Models/
    Plugin.php                   â† Eloquent model with getSettings/saveSettings helpers
    PluginOption.php             â† Options schema rows
    PluginSetting.php            â† Saved values rows
  Providers/
    PluginServiceProvider.php    â† Registers PluginManager singleton, boots built-ins

plugins/                         â† Drop-in external plugin folder (root level)
  .gitkeep
  my-custom-plugin/              â† Example drop-in plugin
    plugin.json
    MyCustomPlugin.php

resources/views/
  plugins/display/
    slideshow.blade.php          â† Swiper.js slideshow Blade renderer
    featured-items-grid.blade.php   â† Featured items: grid layout
    featured-items-list.blade.php   â† Featured items: list layout
    featured-items-slider.blade.php â† Featured items: Swiper slider layout
  livewire/
    admin-plugins.blade.php      â† Admin plugin manager UI

database/
  migrations/
    2026_07_18_100000_create_plugins_tables.php
  seeders/
    PluginSeeder.php             â† Seeds Slideshow + FedEx with all plugin_options

bootstrap/
  providers.php                  â† PluginServiceProvider registered here
```
...
Display plugins are embedded in CMS page content or product descriptions using the `[plugin:slug]` shortcode format â€” similar to WordPress shortcodes.
...
4. `plugin->render(array $params, Plugin $dbRecord)` is called â€” `$params` contains all key=value pairs from the shortcode.
...
| **API** | FedEx REST API v1 â€” OAuth2 client credentials |
...
> âš ï¸ **SOAP credentials are NOT compatible.** The old FedEx Web Services SOAP API has been retired. You must register at [developer.fedex.com](https://developer.fedex.com) to obtain REST API credentials.

**Settings (configured in Admin â†’ Plugins â†’ FedEx â†’ Settings tab):**
...
| `FedEx_NorthAmerica` | Enable N. America Rates | No | Toggle checkbox â€” quote domestic US/CA/MX shipments |
| `FedEx_International` | Enable International Rates | No | Toggle checkbox â€” quote international shipments |
...
| **API** | UPS REST Rating API v2205 â€” OAuth2 client credentials |
...
> ðŸ’¡ The UPS REST API uses a `Shop` endpoint that returns **all eligible services in a single request** â€” there is no per-service round-trip. The plugin then filters by each service's enable/disable toggle before displaying rates to the customer.

**Settings (configured in Admin â†’ Plugins â†’ UPS â†’ Settings tab):**

| Setting Key | Label | Required | Description |
|---|---|---|---|
| `UPS_Client_ID` | UPS Client ID (OAuth2) | Yes | From developer.ups.com â†’ My Apps |
| `UPS_Client_Secret` | UPS Client Secret (OAuth2) | Yes | Keep confidential â€” never expose in front-end code |
...
|---|---|---|
| `UPS_Ground` | UPS Ground | 1â€“5 business days |
...
| **API** | USPS REST API v3 â€” OAuth2 client credentials |
...
> âš ï¸ **The legacy USPS Web Tools XML/SOAP API was deprecated January 2024.** Old credentials obtained from the original USPS registration are NOT compatible with the v3 REST API. You must create a new account at [developer.usps.com](https://developer.usps.com).

> ðŸ’¡ Unlike UPS and FedEx, USPS quotes one service at a time. The plugin queries each enabled service in parallel (separate HTTP calls per service). Disabled services are skipped â€” enable only the services relevant to your operation to minimize API calls and page load time.

**Settings (configured in Admin â†’ Plugins â†’ USPS â†’ Settings tab):**

| Setting Key | Label | Required | Description |
|---|---|---|---|
| `USPS_Client_ID` | USPS Client ID (OAuth2) | Yes | From developer.usps.com â†’ My Apps |
...
|---|---|---|
| `USPS_Priority_Mail` | Priority Mail | 1â€“3 days |
| `USPS_Priority_Mail_Express` | Priority Mail Express | Overnightâ€“2 days |
| `USPS_Ground_Advantage` | USPS Ground Advantage | 2â€“5 days |
| `USPS_First_Class_Package` | First-Class Package Service | 2â€“3 days (â‰¤ 15.99 oz) |
| `USPS_Parcel_Select` | Parcel Select | 2â€“9 days (economy) |
| `USPS_Parcel_Select_Lightweight` | Parcel Select Lightweight | 2â€“9 days (very light) |
| `USPS_Priority_Mail_Cubic` | Priority Mail Cubic | 1â€“3 days (small dense packages) |
...
| **Engine** | Swiper.js (CDN, loaded with `@once` â€” safe to use alongside the slideshow plugin) |
| **Activation Required** | No (active by default) |
| **Data source** | Products with `featured_item = 1` â€” toggled per-product in Admin |

The Featured Items plugin renders your hand-picked featured products into any CMS page. Supports three display modes â€” **grid**, **list**, and **Swiper slider** â€” all visually consistent with the main `/shop` catalog design.

#### Marking a Product as Featured

1. Go to **Admin â†’ E-Commerce â†’ Products** and open any product.
2. Click **Advanced Settings** in the section nav.
3. Enable the **â˜… Featured Item** toggle.
...
All parameters are optional. Plugin-level defaults (configurable in Admin â†’ Plugins â†’ Featured Items â†’ Settings) apply when a parameter is omitted from the shortcode.

| Parameter | Default | Description |
|---|---|---|
| `display` | `grid` | Display mode: `grid`, `list`, or `slider` |
| `max` | `12` | Maximum number of featured products to show (1â€“100) |
...
- **â˜… Featured** amber badge on the product image
- **Sale** badge when `on_sale = 1`

#### Swiper & Slideshow Plugin Coexistence

The slider view uses `@once` to load Swiper CSS and JS from CDN. If the **Slideshow Plugin** is also on the same page, Swiper is only loaded once â€” there is **no conflict or double-loading**. Each slider instance gets a unique auto-generated ID (`fi_xxxxxxxx`) so multiple Featured Items shortcodes on the same page work independently.

#### Admin-Configurable Settings

Default values for all parameters can be set globally in **Admin â†’ Plugins â†’ Featured Items Display â†’ Settings**:
...
1. Calculates the cart weight (sum of `item_qty Ã— item_weight` for all shippable items)
...
4. Displays the combined list as radio buttons â€” the customer picks their preferred service
5. The selected option's amount and name are used in `calculateTotals()` and written to the order record at placement

```
Cart Items (weights) â”€â”€â–º  buildShippingOptions()
 Customer Address     â”€â”€â–º    â”œâ”€â”€ ShippingCalculationService (flat rates)
                             â””â”€â”€ PluginManager::getShippingRates(ShippingContext)
                                   â”œâ”€â”€ FedExPlugin::getRates()  (if activated)
                                   â”œâ”€â”€ UpsPlugin::getRates()    (if activated)
                                   â””â”€â”€ UspsPlugin::getRates()   (if activated)
                                           â”‚
                             Merged + sorted lowâ†’high
                                           â”‚
                             Radio buttons on /checkout/review
```

> **Graceful degradation**: If a carrier API is unreachable or returns an error, that plugin silently returns an empty array and logs to `storage/logs/laravel.log`. The customer still sees all other available options â€” checkout is never blocked by a shipping API failure.

---

<a name="fedex-setup"></a>
### FedEx Setup â€” Step by Step
...
3. Navigate to **My Projects** â†’ **Create a Project**
4. Select **Shipping** â†’ **Rate API** and complete the project creation
...
- In your FedEx profile at [fedex.com](https://www.fedex.com) â†’ Account Management
...
3. In the **Settings** tab, enter:
   - **FedEx Account Number** â€” your 9-digit account
   - **API Client ID** â€” from developer.fedex.com
   - **API Client Secret** â€” from developer.fedex.com
   - **Origin ZIP Code** â€” your warehouse/ship-from ZIP (set via FedEx_markup field)
   - **Rate Markup** â€” `0.00` for pass-through, or a dollar amount to add
...
<a name="ups-setup"></a>
### UPS Setup â€” Step by Step

#### 1. Create a UPS Developer Account

1. Go to [developer.ups.com](https://developer.ups.com)
2. Click **Get Started** â†’ **Sign In / Register** using your UPS.com account (or create one)
3. Navigate to **My Apps** â†’ **Add App**
...
> ðŸ’¡ **Negotiated rates**: If you have a UPS account number and want negotiated/discounted rates instead of retail rates, enter your account number in the `UPS_Account_Number` setting. Without an account number, the plugin returns published retail rates.
...
3. In the **Settings** tab, enter:
   - **UPS Client ID** â€” from developer.ups.com
   - **UPS Client Secret** â€” from developer.ups.com
   - **UPS Account Number** â€” optional, for negotiated rates
   - **Origin ZIP Code** â€” your warehouse/ship-from ZIP
   - **Origin Country Code** â€” 2-letter ISO (e.g. `US`)
   - **Rate Markup** â€” `0.00` for pass-through, or a dollar amount
...
<a name="usps-setup"></a>
### USPS Setup â€” Step by Step

> âš ï¸ **Important**: The old USPS Web Tools API (using the `UserId` XML parameter) was deprecated on **January 15, 2024**. This plugin uses the new [USPS APIs REST portal](https://developer.usps.com). Old credentials will **not** work â€” you must register at the new portal.
...
3. Once logged in, click **My Apps** â†’ **Add App**
4. Under **APIs**, select:
...
3. In the **Settings** tab, enter:
   - **USPS Client ID** â€” Consumer Key from developer.usps.com
   - **USPS Client Secret** â€” Consumer Secret from developer.usps.com
   - **Origin ZIP Code** â€” your 5-digit US warehouse ZIP (required â€” USPS only ships from US)
   - **Rate Markup** â€” `0.00` for pass-through, or a dollar amount
...
- A typical store needs 2â€“3 services (e.g. Ground Advantage + Priority Mail + Priority Mail Express)
- Each additional enabled service adds ~200â€“400ms to the checkout review page load
...
- USPS only supports international shipping from US origin â€” if your origin country is not US, disable this plugin for international customers and use FedEx or UPS instead
...
- Common issues: invalid ZIP code in `USPS_From_Zip`, or selecting First-Class for packages over 15.99 oz (the USPS API returns an error for ineligible combinations â€” the plugin silently skips those)
...
The active toggle saves immediately via Livewire â€” no page reload or save button required.

### Settings Panel

Clicking **Settings** on any row opens a slide-in right panel with three tabs:

**Settings Tab** â€” data-driven form rendered from `plugin_options` rows:

| `field_type` | `field_editor` | Renders As |
|---|---|---|
| `input` | â€” | Monospace text input |
| `textarea` | â€” | Plain resizable textarea |
| `textarea` | `css` | Dark code editor (slate-900 bg, green monospace text) |
| `checkbox` | â€” | Animated on/off toggle switch |
| `select` | â€” | Dropdown (values from `field_selections` comma list) |
| `text-only` | â€” | Read-only `<pre><code>` reference block |

**Usage Tab** â€” shortcode with one-click copy button, `usage_instructions` HTML rendered, external docs link.

**Activation Tab** â€” only shown when `activation_required = yes`. Shows activation instructions, key entry field, and activate/deactivate controls.
...
**Shipping plugin** (`app/Plugins/Shipping/MyShipper.php`) â€” implement `getRates()` and return:
```php
return [
...
### Step 2 â€” Register in PluginServiceProvider
...
### Step 3 â€” Seed the Database Record
...
Drop-in plugins are placed in the root `/plugins/` directory and discovered automatically on every application boot â€” no code changes to the platform are required.

### Folder Structure
```
plugins/
  my-custom-plugin/
    plugin.json          â† Required manifest
    MyCustomPlugin.php   â† PHP class (must match "class" value in plugin.json)
```
...
3. Calls `syncExternalPlugin()` â€” upserts the `plugins` DB record and its `plugin_options` rows
...
> **Re-running the seeder is safe.** All records use `updateOrCreate` â€” running the seeder on an existing install will update metadata and add any new options without overwriting existing saved settings values.
...
All carrier credentials (FedEx Client ID/Secret, UPS Client ID/Secret, USPS Client ID/Secret) are stored in the `plugin_settings` table via the Admin Panel â€” **not** in `.env`. No environment variables are needed for the plugin system itself.
...
The **CMS Code / Video Embed Manager** provides a site-wide library of reusable HTML and video embed snippets that can be inserted into any CMS page, product description, or list menu item using a simple shortcode. The embed code is stored centrally in the database and is **never exposed to the TinyMCE editor** â€” this solves two critical problems:

1. **TinyMCE Protection** â€” Rich-text editors routinely reformat, strip attributes from, or entirely remove HTML they do not recognise (iframes, custom scripts, third-party widgets). By storing embed code in a dedicated table and inserting it only at render time via shortcode, the code is completely invisible to the editor and is never modified.

2. **Single-Source Updates** â€” If the same video or widget is embedded on ten different pages, you only need to update it in one place. Every page that references the shortcode will automatically reflect the change on next load.

---

### 1. Database Table â€” `cms_embeds`

| Column | Type | Default | Description |
|---|---|---|---|
| `id` | `bigint` (PK, auto) | â€” | Unique embed ID |
| `name` | `varchar(255)` | â€” | Internal admin label â€” never shown publicly |
| `embed_type` | `tinyint unsigned` | `0` | `0` = YouTube Â· `1` = Vimeo Â· `2` = Other HTML |
...
**Access points:**
- **Top nav CMS dropdown** â†’ **Code Embeds** (active-highlighted when on any `admin.cms-embeds.*` route)
- **Responsive/mobile nav** â†’ CMS Sections â†’ **Code Embeds**
- **CMS Admin Hub sidebar** (`/admin/cms`) â†’ **Code Embeds** with `</>` code-bracket icon

#### Index Page (`AdminCmsEmbeds`)
Displays all embed records in a searchable, filterable table:
- **Search** â€” filters by name
- **Type filter** â€” All / YouTube / Vimeo / Other HTML
- **Active filter** â€” All / Active / Inactive
- **Inline active toggle** â€” enable/disable without entering the edit form
- **Shortcode preview** â€” displays `[code-embed:{id}]` in monospace for easy copying
- **Type badge** â€” colour-coded: ðŸ”´ YouTube Â· ðŸ”µ Vimeo Â· â¬œ Other HTML
- **Delete** â€” with confirmation prompt

#### Edit Form (`AdminCmsEmbedEdit`)
Two-panel layout:

**Left panel â€” Details:**
- `Name` â€” internal admin label
- `Active` â€” toggle; inactive embeds render silently as `<!-- [embed-inactive: N] -->`
- `Embed Type` â€” three radio buttons (YouTube / Vimeo / Other HTML) with contextual help text per type
- **Shortcode badge** (existing records) â€” shows `[code-embed:{id}]` with a one-click Copy button and the optional label variant `[code-embed:{id} label="..."]`

**Right panel â€” Code Snippet:**
- Raw **monospace `<textarea>`** â€” intentionally **not** connected to TinyMCE. Protects the code from reformatting.
- Contextual placeholder and help text changes with the selected type.
- **Live Preview** â€” renders below the textarea:
  - YouTube/Vimeo â†’ shown inside the responsive 16:9 wrapper
  - Other HTML â†’ shown with a safety notice (admin-only content)
...
### 4. Shortcode Processing â€” Pipeline Integration

`[code-embed:N]` is resolved by **Pipeline B** (`ContentParserService::parse()` â†’ `ShortcodeProcessor::process()`). It runs as **Pass 1a**, before the existing download and plugin passes:

```
Pass 1a â†’ [code-embed:N]    â† NEW
Pass 1b â†’ [download:N]
Pass 2  â†’ [plugin:slug]
```
...
|---|:---:|
| CMS Page â€” main body | âœ… Pipeline B |
| CMS Page â€” left sidebar | âœ… Pipeline B |
| CMS Page â€” right sidebar | âœ… Pipeline B |
| Product short description | âœ… Pipeline B |
| Product long description | âœ… Pipeline B |

> See the **Shortcode Support Matrix** section for the full cross-shortcode comparison.

---

### 5. Rendering â€” Three Output Modes

`ShortcodeProcessor::renderCodeEmbed()` selects one of three strategies based on `embed_type`:

#### Mode A â€” Responsive Video Wrapper (YouTube / Vimeo)
...
> **CSS deduplication:** The `<style id="cms-embed-css">` block is emitted only once per PHP request via a `static bool $embedCssLoaded` flag on `ShortcodeProcessor`. If multiple YouTube/Vimeo embed shortcodes appear on the same page, only the first emits the CSS â€” identical to the Video.js deduplication pattern used by the CMS Downloads feature.

#### Mode B â€” Raw HTML (Other)
...
#### Mode C â€” Inactive / Empty
- **Inactive record** â†’ `<!-- [embed-inactive: N] -->`
- **Empty snippet** â†’ `<!-- [embed-empty: N] -->`
- **Render error** â†’ `<!-- [embed-error: N] -->` (also logged via `Log::error`)

---

### 6. Model â€” `App\Models\CmsEmbed`
...
### 7. Adding a New Embed â€” Step-by-Step

1. Go to **Admin â†’ CMS â†’ Code Embeds** (top nav dropdown or CMS sidebar)
2. Click **New Embed**
3. Enter an internal **Name** (e.g. `"Homepage Intro Video"`)
4. Select the **Embed Type** (YouTube, Vimeo, or Other HTML)
5. Paste the full embed code (e.g. the `<iframe>` from YouTube's Share â†’ Embed panel) into the **Code Snippet** textarea
6. Click **Save Embed**
7. Copy the generated shortcode (e.g. `[code-embed:5]`) from the **Shortcode** panel
8. Paste the shortcode into any CMS page body, sidebar column, or product description â€” it will render the embed when the page loads
...
|---|---|---|
| YouTube tutorial video | YouTube | Paste YouTube `<iframe>` â€” responsive wrapper applied automatically |
| Vimeo product showcase | Vimeo | Paste Vimeo `<iframe>` â€” responsive wrapper applied automatically |
...
The Event Details feature integrates event scheduling metadata directly into the **product variant** level. Rather than maintaining a separate events table linked to a product, each event date/session is modelled as its own **product variant** â€” with its own price, SKU, sale price, and inventory count. This design solves the classic pricing-per-date problem: the product itself is the "event series" (e.g. *Digital Marketing Seminar*) and each variant is an individual session (e.g. *Sept 16 â€” 9 AM*, *Sept 24 â€” 1 PM*, *Oct 1 â€” 2 PM*).
...
| ariant_id | bigint FK | Unique â€” constrained to product_variants, cascades on delete |
...
- **pp/Models/ProductVariantEvent.php** â€” New model. elongsTo(ProductVariant), full \, datetime casts on event_start_date and event_end_date.
- **pp/Models/ProductVariant.php** â€” Updated: is_event added to \ and \ (boolean). New eventDetails() hasOne(ProductVariantEvent) relationship.
- **pp/Livewire/AdminProductEdit.php** â€” Updated: 10 new public properties for event fields. All variant lifecycle methods updated:
  - 
esetVariantForm() â€” resets event fields to defaults
  - startEditVariant() â€” loads event data from \->eventDetails
  - saveVariant() â€” creates/updates ProductVariantEvent if is_event, deletes it otherwise
  - updateVariant() â€” upserts or deletes event record; validates event_start_date and event_label as required when is_event is enabled
  - duplicateVariant() â€” copies event record to duplicated variant
  - loadProduct() â€” eager-loads ariants.eventDetails alongside ariants.inventory
- **
esources/views/livewire/partials/variant-management.blade.php** â€” Updated: Event Details sub-section added to both the **Edit Variant** and **Create Variant** forms. Event badge column added to the variant list table.
...
|---|---|---|
| Event Label | âœ… Yes | Primary text shown on the calendar tile |
| Alternate / Tooltip Label | No | Secondary label, used for tooltips or longer descriptions |
| Event Start Date & Time | âœ… Yes | datetime-local picker |
...
| Show Date on Front-End | No | Checkbox â€” hide/show the date on the public calendar renderer |
...
- event_start_date â€” must be present
- event_label â€” must be a string, max 255 characters
...
- is_event â€” filter flag to identify event variants
- event_start_date / event_end_date â€” date range for the calendar tile
- event_label â€” tile display text
- lternate_label â€” tooltip text
- label_background â€” tile background/border colour
- show_date â€” whether to render the date string
- event_location â€” venue or URL shown below the event label
- event_sort â€” ordering within a day or date range
...
- [product:N] â€” embeds the full product (all variants, including event sessions) anywhere a shortcode is accepted
...
- **Product Page** (display_on_item_view) â€” shown in a list, grid, or scroller below the product detail view
- **Post Add-to-Cart Page** (display_on_post_cart) â€” shown on the intermediary confirmation page after the customer adds the item to their cart
...
- **pp/Models/ProductCrossSell.php** â€” New model. product() and crossSellProduct() BelongsTo relationships, full \, boolean casts.
- **pp/Models/Product.php** â€” Updated: crossSells() HasMany relationship (ordered by sort_order, then id).
- **pp/Livewire/AdminProductEdit.php** â€” Updated:
  ...
  - New methods:
    - updatedCrossSellSearch() â€” live search, debounced, max 25 results, excludes current product and already-added products
    - ddCrossSell(int \) â€” enforces max 10, determines next sort order, resets search state
    - 
emoveCrossSell(int \) â€” scoped to current product for security
    - 	oggleCrossSellItemView(int \) â€” toggles display_on_item_view
    - 	oggleCrossSellPostCart(int \) â€” toggles display_on_post_cart
    - updateCrossSellOrder(int \, float \) â€” updates sort weight
- **
esources/views/livewire/partials/cross-selling.blade.php** â€” New blade partial
- **
esources/views/livewire/admin-product-edit.blade.php** â€” Updated: "Cross-Selling" quick-nav link added; partial included between the Customizations section and the Reviews section
...
| Column | Description |
|---|---|
| Sort | Numeric input â€” change and blur to update sort order |
...
**Limits & Guards**
- Maximum **10** cross-sell items per product â€” search input is hidden and a warning banner is shown when the limit is reached.
...
`php
// Product detail page â€” items shown below the product description
...
| **Engine** | Swiper.js (CDN, loaded with `@once` â€” safe alongside Slideshow and Featured Items plugins) |
| **Activation Required** | No (active by default) |
| **Data source** | `product_cross_sells` table â€” managed per product in Admin â†’ Edit Product â†’ Cross-Selling |

The Cross-Sell List plugin displays the cross-selling products associated with a specific product on any CMS page, or on the post-cart intermediary page via embedded shortcode. It is a purpose-built sibling to the Featured Items plugin and shares the same three display modes â€” **grid**, **list**, and **Swiper slider** â€” with an additional required `product_id` parameter that scopes the output to a single product's cross-sell list.
...
4. For each entry, configure:
   - **Show on Product Page** â€” displays the item in the "You may also like" section on the public product detail page.
   - **Show on Post-Cart Page** â€” triggers the post-cart intermediary page and includes this item in the plugin output rendered there.
   - **Sort Order** â€” numeric weight controlling display order.
...
`product_id` is the only required parameter. All others are optional and fall back to plugin-level defaults configured in **Admin â†’ Plugins â†’ Cross-Sell List Display â†’ Settings**.

| Parameter | Default | Description |
|---|---|---|
| `product_id` | *(required)* | Numeric ID of the product whose cross-sells to display |
| `display` | `grid` | Display mode: `grid`, `list`, or `slider` |
| `max` | `12` | Maximum number of cross-sell products to show (1â€“100) |
| `cols` | `4` | Grid columns in grid mode: `2`, `3`, `4`, `5`, or `6` |
...
> **Note:** The post-cart redirect also overrides the **Go Direct to Checkout** advanced setting on the product â€” cross-sell display always takes priority over both the modal and the direct-checkout bypass.
...
The `product_id` can also be set as the **Default Product ID** in plugin settings (Admin â†’ Plugins â†’ Cross-Sell List Display â†’ Settings), allowing the shortcode to be used without the parameter on single-purpose pages.

---

### Admin-Configurable Settings

Default values for all parameters are set globally in **Admin â†’ Plugins â†’ Cross-Sell List Display â†’ Settings**:
...
| File | Purpose |
|---|---|
| `app/Plugins/Display/CrossSellListPlugin.php` | Plugin class â€” parses shortcode attributes, queries `product_cross_sells`, renders the correct view |
...
| `resources/views/livewire/partials/cross-selling.blade.php` | Admin partial â€” cross-sell management panel on product edit page |
...
| Brand | Description |
|---|---|
| Prestige Design | Main demo brand â€” used across jewelry and apparel |
...
| â†’ Rings | Sub-category of Custom Jewelry |
| â†’ Bracelets | Sub-category of Custom Jewelry |
| â†’ Necklaces | Sub-category of Custom Jewelry |
| â†’ Earrings | Sub-category of Custom Jewelry |
| Watches | Top-level |
...
| 3 | Diamond Mosaic Ring | Size variants | 7 (Size 5â€“8) |
...
| 27 | Men's Titanium Watch (Sweatshirt) | Size+Colour variants | 15 (3 colours Ã— 5 sizes) |
| 28 | Product Builder Example | Session variants | 2 (1hr/2hr) |
| 29 | Donation \| "Make An Offer" Example | Donation tiers | 5 (\â€“\) |
...
**Also included:**
- âœ… **Inventory records** for every variant (with realistic available/reserved stock quantities)
- âœ… **Event records** for all event-type variants (start/end times, location, colour labels)
- âœ… **Product attributes** (product_fields + product_field_options) for multi-variant products
- âœ… **CDN images** for all products â€” served from https://d23w3zagfzgqcb.cloudfront.net/demo/
- âœ… **36 cross-selling relationships** across the jewelry and apparel categories
...
`ash
# Step 1 â€” Run all pending migrations (if not already done)
php artisan migrate

# Step 2 â€” Run the demo store seeder
php artisan db:seed --class=DemoStoreSeeder
`

The seeder is **fully idempotent** â€” running it multiple times is safe. It skips any product whose seo_slug already exists. If the is_demo flag is missing on existing records (e.g., the seeder was run before the flag migration), it will automatically backfill the flag.

**What the seeder does NOT do:**
- It does not copy image files to your server â€” images are served via CDN URLs.
...
2. Navigate to **Settings** (Admin â†’ Settings).
...
9. The amber banner disappears automatically â€” confirming all demo content has been removed.
...
The setting lives in **Admin â†’ Settings â†’ Shop Display** and is persisted in the `cms_settings` table under the key `product_image_orientation`.
...
**16:9 Widescreen** fills the frame at a 16:9 ratio and uses `object-cover` â€” images are cropped to fill the container width. This is the default and matches the original storefront design.

**1:1 Square** uses a perfect-square container with `object-contain` â€” images are scaled to fit within the square without any cropping or distortion, preserving the full image regardless of its proportions.
...
| Location | File |
|----------|------|
| Shop listing page â€” grid view | `resources/views/livewire/shop-catalog.blade.php` |
| Shop listing page â€” list view | `resources/views/livewire/shop-catalog.blade.php` |
| Product detail page â€” main gallery image | `resources/views/livewire/partials/product-gallery.blade.php` |
| Product detail page â€” "You May Also Like" carousel | `resources/views/livewire/product-details.blade.php` |
| Cross-sell widget â€” grid | `resources/views/plugins/display/cross-sell-list-widget-grid.blade.php` |
| Cross-sell widget â€” list | `resources/views/plugins/display/cross-sell-list-widget-list.blade.php` |
| Cross-sell widget â€” slider | `resources/views/plugins/display/cross-sell-list-widget-slider.blade.php` |
| Featured Items plugin â€” grid | `resources/views/plugins/display/featured-items-grid.blade.php` |
| Featured Items plugin â€” list | `resources/views/plugins/display/featured-items-list.blade.php` |
| Featured Items plugin â€” slider | `resources/views/plugins/display/featured-items-slider.blade.php` |
| Featured Items widget â€” grid | `resources/views/plugins/display/featured-items-widget-grid.blade.php` |
| Featured Items widget â€” list | `resources/views/plugins/display/featured-items-widget-list.blade.php` |
| Featured Items widget â€” slider | `resources/views/plugins/display/featured-items-widget-slider.blade.php` |
...
Changes take effect immediately on the next page load. The settings cache is cleared automatically on save â€” no server restart or deployment step is needed.
...
1. **AdminSettings.php** â€” add `'4:3'` to the `in:` validation rule for `product_image_orientation`.
2. **admin-settings.blade.php** â€” add a new `<label>` radio card in the Shop Display card.
3. **All blade files listed above** â€” extend the ternary logic to handle the new value.
4. **README.md** â€” update this section to document the new option.
...
2. Fill in the **Form Settings** (left column):
   - **Form Name** ï¿½ internal label shown in the admin.
   - **Slug** ï¿½ auto-generated from the name; used in debug comments only (the shortcode uses the ID).
   - **Submit Button Label** ï¿½ text on the submit button (default: "Submit").
   - **Active** ï¿½ toggle to enable or disable the form on the frontend.
3. Configure **After Submission** (left column):
   - **Confirmation Message** ï¿½ plain text or simple HTML shown in place of the form after a successful submit.
   - **Redirect URL** ï¿½ if set, the visitor is redirected here 2 seconds after submitting.
4. Configure **Email Notification** (left column):
   - **Send To Email** ï¿½ recipient address for submission notification emails.
   - **Email Subject** ï¿½ custom subject line (defaults to "New form submission: {Form Name}").
5. Optionally add **Custom CSS** (left column) ï¿½ scoped to `.cms-form-wrap` within the form container.
6. Build fields in the **Form Fields** panel (right column) ï¿½ see Field Types below.
7. Click **Save Form**.
...
Each field supports:
- **Label** ï¿½ the visible field label (required)
- **Instructions** ï¿½ small help text shown below the label
- **HTML Above** ï¿½ full HTML/rich content rendered directly above the field (uses TinyMCE)
- **Required** toggle ï¿½ marks the field as required; exposes:
  - **Validation Rule**: Not Empty, Valid Email, or Numeric Only
  - **Custom Error Message** ï¿½ per-field message displayed below the field on failure
...
The shortcode is expanded server-side by `ShortcodeProcessor` (Pass 0, before all other shortcodes). If the form is inactive or the ID is invalid, the shortcode renders as an empty HTML comment ï¿½ no broken markup.
...
- The form uses a `fetch()` POST request ï¿½ no full page reload.
...
- **Delete** ï¿½ permanently removes the individual submission record.
- **Export CSV** ï¿½ downloads all submissions for that form as a `.csv` file with one column per field.

**Search** ï¿½ filter submissions by IP address or any value in the stored JSON data.
...
Email failures are caught silently and logged to `storage/logs/laravel.log` ï¿½ they do not block the success response shown to the visitor.
...
If `RECAPTCHA_SITE_KEY` and `RECAPTCHA_SECRET` are set in your `.env`, **all embedded forms are automatically protected by reCAPTCHA v3** ï¿½ no extra configuration required.
...
When `RECAPTCHA_SITE_KEY` / `RECAPTCHA_SECRET` are not set (local dev, staging without keys), `RecaptchaService::verify()` returns `true` automatically ï¿½ the form works exactly as if reCAPTCHA were not present. No code changes or feature flags needed.
...
RECAPTCHA_THRESHOLD=0.5   # optional, default 0.5 (0.0ï¿½1.0)
```
...
Each form has an optional **Mailing List Opt-in** feature (Admin ? Forms ? Edit Form ? Mailing List Opt-in card). When enabled, the submitted email address (and name, if tagged) are automatically forwarded to a mailing-list provider on every successful submission ï¿½ no opt-in checkbox is required on the form itself.
...
- **Subscriber Email** ï¿½ the value in this field is sent as the subscriber's email address
- **Subscriber Name** ï¿½ the value in this field is sent as the subscriber's display name (split into first/last on the first space)
- **None** (default) ï¿½ the field is not forwarded to the provider
...
#### Form 1 ï¿½ Contact Us (`[cms-form id=1]`)

| Field | Type | Role | Required |
|---|---|---|---|
| Your Name | Text Input | `name` | Yes (not empty) |
| Email Address | Text Input | `email` | Yes (valid email) |
| Comments / Message | Textarea | ï¿½ | Yes (not empty) |
| How did you hear about us? | Dropdown | ï¿½ | No |

- Auto opt-in is **enabled** (provider and list ID left blank ï¿½ configure in admin when ready)
- Submit button label: **Send Message**
- Confirmation: "Thank you for reaching out! We'll get back to you within one business day."

#### Form 2 ï¿½ Email Subscribe (`[cms-form id=2]`)
...
### Overview
Admin-only feature (role_id = 3 / Admin) accessible via **Admin â†’ Checkout â†’ Processors & Payments**. Allows injection of fully customizable form fields into two specific positions in the checkout flow without requiring shortcodes.
...
Each field supports:
- **Label** (required) â€” visible field label
- **Instructions** â€” small helper text displayed below the label
- **Required toggle** â€” mark a field as mandatory before the customer can proceed
- **Validation type** â€” Not Empty / Valid Email / Numeric
- **Custom error message** â€” shown below the field on validation failure
- **HTML Above** â€” optional rich HTML block rendered above the field (supports TinyMCE markup)
- **Active toggle** â€” disable a field without deleting it
- **Reorder** â€” â–² â–¼ buttons to change display order

### Billing Page â€” User Type Filtering
...
|---|---|
| Checkout (Step 1) | `Checkout::saveDetailsAndContinue()` â€” blocks redirect to Step 2 |
| Billing Page (Step 2) | **Top** of `OrderReview::placeOrder()` â€” before any payment gateway call, ensuring required fields can never be skipped after payment |
...
The **Checkout Mailing List Opt-in** panel (same page in the admin) configures automatic or manual newsletter subscription on order completion. The customer's **name and email come from their checkout account** â€” no extra fields are required.
...
- **Checkbox Label** â€” customizable text (e.g. "Yes, add me to the mailing list")
- **Checkbox Position** â€” choose whether the checkbox appears at Step 1 (Checkout) or Step 2 (Billing Page)
...
| Table | Change |
|---|---|
| `checkout_custom_fields` | New table â€” stores all field definitions |
...
- ll ï¿½ Everyone (default)
- guests_only ï¿½ Not logged in only
- uth_only ï¿½ Logged-in users only
- wholesale_only ï¿½ Wholesale users only
...
###  Enhancements (Alt Fields, Dashboard Widget, Anchors, and Centered Galleries)
...
- **Actions**:
  ...
  - **View Site â†—**: Opens the public product view in a new browser tab.
...
- **Badges**: The admin variant list displays all uploaded images with corresponding status badges (e.g. `ðŸ” Search Image`, `ðŸ‘ï¸ Active`, `ðŸš« Inactive`).
...
## Directory Structure & Blade View Architecture
...
app/
â”œâ”€â”€ Http/
â”‚   â”œâ”€â”€ Controllers/        # Standard HTTP controllers (PageController, SocialAuthController, etc.)
â”‚   â””â”€â”€ Middleware/         # Custom middleware (EnsureUserIsAdmin, SetLocale, etc.)
â”œâ”€â”€ Livewire/               # All Livewire components (admin + storefront)
â”œâ”€â”€ Models/                 # Eloquent entities (Product, CmsPage, Ticket, Language, etc.)
â”œâ”€â”€ Plugins/                # Plugin contracts, display plugins, PluginManager
â”œâ”€â”€ Services/               # Business logic (TranslationService, SiteLabelService, etc.)
â”œâ”€â”€ Traits/                 # Reusable model traits (HasTranslations)
â””â”€â”€ helpers.php             # Global helper functions (siteLabel(), etc.)
config/                     # Custom configurations (nav_schemes, payment_processors)
database/
â”œâ”€â”€ migrations/             # All database schemas
â””â”€â”€ seeders/                # Core, language, and demo seeders
plugins/                    # Drop-in external plugins
resources/
â”œâ”€â”€ css/                    # Tailwind CSS customizations & variables
â””â”€â”€ views/                  # Blade templates â€” see Blade Architecture section below
routes/
â”œâ”€â”€ web.php                 # All public + admin routes
â””â”€â”€ auth.php                # Authentication routes (Livewire Volt)
```

---

## ðŸ—‚ï¸ Blade View Architecture
...
### Tier 1 â€” Full-Page Blades (`pages/` and `user/`)

These files render a **complete HTML document** â€” they include their own `<!DOCTYPE html>`, `<head>`, header component, footer component, and `</html>`. They are returned directly from a controller or `Route::view()`. They are **not** Livewire components and do not use `#[Layout(...)]`.

```
resources/views/
â”œâ”€â”€ pages/                         # Public-facing standalone HTML pages
â”‚   â”œâ”€â”€ home.blade.php             # GET /  â€” Home page. Loads the 'home' CmsPage from the
â”‚   â”‚                              #         database (with translations). Rendered by Route::view.
â”‚   â”œâ”€â”€ cms.blade.php              # GET /{slug}  â€” All other CMS content pages, served by
â”‚   â”‚                              #   PageController::show(). Receives $page (CmsPage model with
â”‚   â”‚                              #   translations loaded). Supports left/right sidebars, header
â”‚   â”‚                              #   images, plugin shortcodes, and tag display.
â”‚   â”œâ”€â”€ cms-category.blade.php     # GET /category/{slug}  â€” Category archive listing.
â”‚   â”œâ”€â”€ cms-tag.blade.php          # GET /tag/{slug}  â€” Tag archive listing.
â”‚   â””â”€â”€ cms-password.blade.php     # Served by PageController when a page has access controls
â”‚                                  #   (purchase gate or access code) that are not yet satisfied.
â”‚
â””â”€â”€ user/                          # Authenticated user pages (auth middleware enforced in routes)
    â”œâ”€â”€ profile.blade.php          # GET /profile  â€” User profile management. Contains
    â”‚                              #   Livewire profile sub-components (update info, password,
    â”‚                              #   delete account). Uses x-app-layout wrapper component.
    â””â”€â”€ dashboard.blade.php        # Legacy Breeze boilerplate kept for reference. The active
...
## ðŸ“œ Demo Store Seeding & Administration
...
## E-Commerce: Storefront & Shopping
...
## Digital Downloads â€” Product Variants
...
## ðŸ“¦ 10. Bulk Product & Variant Import System
...
### ðŸ› ï¸ Key Capabilities & Technical Breakdown
...
## ðŸ“‹ 12. Admin Product COPY / Duplication Feature
...
## CMS System: Pages, Posts & Content

The CMS includes a premium database-driven page management system. The full feature set for managing CMS pages is documented inline within [E-Commerce: Administration](#e-commerce-administration) above under **Dynamic CMS Pages & Security Gates**.
...
## ðŸ” 15. CMS Page Access Gating

A flexible, dual-mechanism access control system for individual CMS pages. Pages can be restricted by **product purchase verification**, an **access code**, or **both simultaneously**. The two gates operate independently â€” satisfying either one is sufficient when both are active.
...
- Unlocked codes are stored in `session('unlocked_access_codes')` as an array â€” a visitor can unlock multiple code-gated pages per session.
...
Navigate to **Admin â†’ Products â†’ Edit Product â†’ Advanced Settings** and scroll to the **Post-Order Completion Redirect** section.
...
> **Note:** The button only appears for line items whose product has `completion_redirect` set. If a customer orders multiple products, only the products with a redirect configured will show the button â€” other items display normally.

---

### ðŸ§© CMS Page Shortcode Reference
...
1. Navigating to **Admin â†’ CMS â†’ Pages**
...
The optional `label` attribute in the shortcode (`[page:5 label="Welcome Page"]`) is **ignored** by the redirect resolver â€” the label is only used in the TinyMCE Shortcode Generator for display purposes. The system always resolves the ID to the page's actual SEO slug URL.
...
Both columns are nullable. If both are `NULL`, there is **no behavioural change** to the order flow â€” the default confirmation page is shown and no extra button appears in emails.
...
| File | Role |
|---|---|
| `app/Models/Product.php` | `resolveCompletionUrl(?string $raw): ?string` â€” static helper that resolves raw values to absolute URLs (single source of truth) |
| `app/Models/Product.php` | `completionRedirectLabel(): string` â€” instance method returning the label with `"View Content"` fallback |
| `app/Livewire/OrderReview.php` | `resolveCompletionRedirect(Order $order)` â€” called after order placement to determine the customer's redirect destination |
| `app/Livewire/OrderReview.php` | Order confirmation email builder â€” injects "View Content" button per line item |
| `app/Livewire/AdminOrderDetails.php` | Shipment confirmation, download reminder, and admin order confirmation resend email builders â€” each injects "View Content" button per line item |
...
| `resources/views/livewire/admin-product-edit.blade.php` | Advanced Settings panel UI â€” destination field, button label field, live preview, and active-redirect badge |
...
#### Example A â€” Redirect to an External URL
...
1. Go to **Admin â†’ Products â†’ Edit** your course product.
2. Scroll to **Advanced Settings â†’ Post-Order Completion Redirect**.
...
#### Example B â€” Redirect to a Gated CMS Page (Purchase-Verified Access)
...
This pattern combines two separate features: the **Post-Order Completion Redirect** (on the product) and the **CMS Page Product Purchase Gate** (on the CMS page). For full details on all gate types, combinations, and the dual-gate UI, see **Â§ 15 â€” CMS Page Access Gating** below.

##### Step 1 â€” Note your Product ID

In **Admin â†’ Products â†’ Edit**, note the numeric product ID from the page URL: `/admin/ecommerce/products/{ID}/edit`.

##### Step 2 â€” Create or edit the destination CMS page

1. Go to **Admin â†’ CMS â†’ Pages** and open (or create) the page for your gated content.
...
##### Step 3 â€” Set the Completion Redirect on the product

1. Go back to **Admin â†’ Products â†’ Edit** for your product.
2. Scroll to **Advanced Settings â†’ Post-Order Completion Redirect**.
...
4. `PageController::show()` runs the purchase gate â€” since the customer just purchased the product, `hasPurchasedProduct()` returns `true` and they are granted access immediately.

**Result:**
- The customer lands directly on the gated content page immediately after payment â€” no extra login or confirmation step.
...
## ðŸ“… 13. Events Calendar Display Plugin (`[plugin:events-calendar-2026]`)
...
## Header & Footer Builder & CSS Manager

## ðŸŽ¨ Header & Footer Layout Builder & CSS Manager
...
#### ðŸ” Core Capabilities & Content Coverage
...
#### âš™ï¸ Modes & Shortcode Parameters
...
#### ðŸš€ API Endpoint & Seeded Search Page
...
## ðŸŽ›ï¸ 9. Advanced Search Filtering System (Shop Catalog)
...
### âš™ï¸ Admin Configuration & Default State
...
### ðŸŽ¨ Storefront Slideout Drawer & Collapsible Panel
...
### Live Search 2026 Built-In Display Plugin
...
## Shortcode System & Parsing Pipelines
...
## Search System & Collated Index Engine

## ðŸ” 11. Collated Search Index Engine & Multi-Content Live Search
...
> The platform uses a `languages` database table and JSON-translated columns on content models (via the `HasTranslations` trait). The `AdminLanguages` Livewire component manages available languages. AI-powered auto-translation is available via the OpenAI integration (TranslationService + TranslateContentJob). A currency override per language is supported via `LanguageService::currencyOverride()`. The language switcher in the public header auto-hides when only one language is configured.
...
## Dynamic Blade & Livewire Content Parser
...
## API & Webhook Endpoints (Routing Directory)
...
## Production Checklist & Upgrading
...
|---|---|---|
| `ContentParserService` | `app/Services/` | Compiles Blade + shortcodes in DB content fields |
| `HeaderFooterCssManager` | `app/Services/` | Compiles and injects dynamic CSS theme tokens into `<head>` |
| `HeaderFooterParserService` | `app/Services/` | Processes header/footer block shortcodes (logos, menus, search) |
| `DiscountService` | `app/Services/` | Priority-based discount calculation engine |
| `TaxCalculationService` | `app/Services/` | Selective taxable subtotal + VAT/Sales Tax engine |
| `ShortcodeProcessor` | `app/Services/` | Pipeline B shortcode resolver (`[download:N]`, `[plugin:slug]`) |
| `ListMenuRenderer` | `app/Services/` | Pipeline A list-menu resolver (`[list:N]`) |
| `SiteLabelService` | `app/Services/` | Multilingual label resolution via `siteLabel()` helper |
| `TranslationService` | `app/Services/` | AI-powered translation via OpenAI |
| `CmsDownloadController` | `app/Http/Controllers/` | Serves CMS Downloads (local, S3, pre-signed URL) |

### Key Models

| Model | Table | Notable Methods / Accessors |
|---|---|---|
| `Product` | `products` | `resolveCompletionUrl()`, `completionRedirectLabel()`, `parsedShortDescription`, `parsedLongDescription` |
| `ProductVariant` | `product_variants` | `getStockForFulfillment($country, $state)`, `inventories()` HasMany |
| `CmsPage` | `cms_pages` | `parsedContent`, `parsedLeftCol`, `parsedRightCol` accessors |
| `CmsSetting` | `cms_settings` | `allCached()`, `resolveLogoUrl()`, `resolvePageBgImageUrl()`, `isAdvancedSearchEnabled()` |
| `Order` | `orders` | UUID-keyed, `order_external_id`, `order_invoice_no` indexes |
| `Language` | `languages` | Controls multilingual switcher, currency overrides |
| `ProductImage` | `product_images` | `resolveUrl()` â€” single central resolver (url_source â†’ CDN â†’ S3 â†’ local) |
...
### Creating a New Display Plugin (Built-in)

1. Create `app/Plugins/Display/MyPlugin.php` implementing `DisplayPluginInterface`.
2. Implement `render(array $params): string` returning the plugin's HTML output.
3. Register in the `plugins` table: `type = 'display'`, `slug = 'my-plugin-2026'`.
4. Embed anywhere using `[plugin:my-plugin-2026 param1=value]` in any CMS page, product description, or header/footer block.

### Creating a Drop-in External Plugin

1. Create a directory under `plugins/my-plugin-2026/`.
2. Add a `plugin.json` manifest with `slug`, `name`, `type`, `class`, and `options` schema.
3. Add your main plugin PHP class inside the directory.
4. The `PluginManager` auto-discovers and registers it on next boot.

### Key Routes Reference

| Route | Handler | Description |
|---|---|---|
| `GET /` | `Route::view('pages.home')` | Home page (CMS page ID 1) |
| `GET /{slug}` | `PageController::show()` | All CMS content pages |
| `GET /shop` | `ShopCatalog` Livewire | Product catalog (`layouts/public`) |
| `GET /items/{seo_link}` | `ProductDetails` Livewire | Product detail page |
| `GET /admin` | `AdminDashboard` Livewire | Admin home (role 3/4/5) |
| `GET /api/live-search-api` | `PluginApiController@liveSearchApi` | Live search JSON API |
| `POST /webhooks/inventory-update` | `InventoryWebhookController` | Inventory sync webhook (no CSRF) |
| `GET /cms-download/{id}` | `CmsDownloadController::serve()` | CMS file delivery |
| `GET /downloads/{orderDetail}/{token}` | `DownloadController` | Secure order download (token-gated) |
| `POST /admin/cms-pages/upload-image` | `CmsImageUploadController` | TinyMCE inline image upload |
...
| `php artisan db:seed --class=DevEcommerceSeeder` | Load demo products/brands/categories |
| `php artisan search:rebuild-index` | Rebuild search index for all CMS pages + products |
| `php artisan search:rebuild-index --force` | Force full rebuild even if indexes are locked |

### Scoping Rules â€” Admin vs. Public

- All dynamic theme styles (`<x-site-theme-styles />`, `<x-header-footer-styles />`) inject **only** into `layouts/public.blade.php`. The admin layout (`layouts/app.blade.php`) is completely isolated.
- Swiper.js CDN loads **only** on `layouts/public.blade.php` (blocking `<script src>` tag). Never on the admin layout.
- Admin-area CSS selectors should be scoped with `body.admin-area` to prevent bleed-over into the public theme.
- Frontend theme overrides should be scoped with `body:not(.admin-area)`.
